...
const problems = Array.from({ length: 50 }, (_, i) => ({ //underscoren er en placeholder for en variable
  index: i + 1, //for at holde styr på hver index af problemet
  problem: `${Math.floor(Math.random() * 10) + 1} ${operators[Math.floor(Math.random() * operators.length)]} ${Math.floor(Math.random() * 10) + 1}` //laver problemet
}));
